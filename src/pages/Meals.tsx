    ...
    <div className="space-y-6">
      ...
      >
        <div className="grid grid-cols-7 gap-1.5 sm:gap-2 mb-2">
          {WEEKDAYS.map((d) => (
            <div key={d} className="text-center text-[10px] sm:text-xs font-extrabold uppercase tracking-wider text-ink/40 py-1">
              {d}
            </div>
          ))}
        </div>
                  ...
                  {(['lunch', 'dinner'] as const).map((meal) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-5 h-5 sm:w-7 sm:h-7 rounded-md sm:rounded-lg text-[9px] sm:text-[11px] font-extrabold transition
                          ${on ? 'bg-emerald-500 text-white' : 'bg-rose-200 text-rose-700'}
                          ${can ? 'hover:scale-110 cursor-pointer' : 'cursor-not-allowed opacity-80'}`}
                      >
                        {meal === 'lunch' ? 'L' : 'D'}
                      </button>
                    )
